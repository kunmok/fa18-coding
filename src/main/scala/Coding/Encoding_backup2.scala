...
//          bufInterleaver(p_cnt - 1.U + puncIndicesWire((o_cnt+i.U) % params.n.U)(((o_cnt+i.U) / params.n.U) % puncMatBitWidth.U)) := AXWires(i.U)(params.m.U)
//        }
//      }
//      p_cnt := p_cnt + puncPolyWire((o_cnt/params.n.U) % puncMatBitWidth.U)
//      o_cnt := o_cnt + params.n.U
//      when(p_cnt >= params.O.U) {
//        p_cnt := 0.U
//      }
//      when(o_cnt === (params.O.U - puncPolyWire((o_cnt/params.n.U) % puncMatBitWidth.U))){
//        state := sDone
//      }
//    }.otherwise{                                  // no puncturing
//      (0 until params.n).map(i => { bufInterleaver(o_cnt + i.U) := AXWires(i.U)(params.m.U) })
//      o_cnt := o_cnt + params.n.U
